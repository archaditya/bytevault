...
func (s *EphemeralService) CreateMultipartUploadSession(
	...
	if size > maxBytes {
		return nil, "", nil, fmt.Errorf("file size exceeds maximum allowed guest limit of %d GB", int(settings.MaxFileSizeGb))
	}
...
func (s *EphemeralService) RequestDownload(ctx context.Context, token string, password *string) (string, error) {
	...
	// Generate 2-hour active presigned URL for browser stream and resumable HTTP Range
	downloadURL, err := s.storage.GeneratePresignedDownloadURL(ctx, share.StorageKey, 2*time.Hour, share.Filename, false)
	if err != nil {
	...
	if shouldBurn {
		...
		// Delay physical R2 storage deletion by 2 hours so active/resumed download streams cleanly
